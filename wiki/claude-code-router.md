...
<span style="color:red">**The sovereignty story.** The most interesting target is an **open-weights** model you run **on-prem** — a DeepSeek coding model claimed near-Sonnet quality at ≈10% of the cost, runnable on a ≈half-million-dollar GPU box ("we don't depend on Trump"). CCR lets you point the same harness — same skills, same hooks — at it. Two caveats: take "it *is* Sonnet on every benchmark" as marketing (the gap is task-dependent), and note that routing to a *hosted* DeepSeek API means your code leaves your perimeter — the on-prem deployment is what actually buys sovereignty. CC-specific features (front-matter, hooks) may not fully port to a non-CC backend.</span>
